...
def initialize(rhapi:RHAPI.RHAPI):
    ...
    elrs_bindphrase = UIField(name = 'comm_elrs', label = 'ELRS BP Bind Phrase', field_type = UIFieldType.TEXT)
    ...
    _heat_name = UIField('_heat_name', "Show Heat Name", desc="Show the heat's name on start", field_type = UIFieldType.CHECKBOX)
    rhapi.fields.register_option(_heat_name, 'elrs_vrxc')

    _round_num = UIField('_round_num', "Show Round Number", desc="Show round number on start", field_type = UIFieldType.CHECKBOX)
    rhapi.fields.register_option(_round_num, 'elrs_vrxc')

    _class_name = UIField('_class_name', "Show Class Name", desc="Show the class's name on start", field_type = UIFieldType.CHECKBOX)
    rhapi.fields.register_option(_class_name, 'elrs_vrxc')

    _event_name = UIField('_event_name', "Show Event Name", desc="Show the event's name on start", field_type = UIFieldType.CHECKBOX)
    ...
    _heatname_row = UIField('_heatname_row', 'Heat Name Row', desc='Use rows between 0-17', field_type = UIFieldType.BASIC_INT, value=2)
    rhapi.fields.register_option(_heatname_row, 'elrs_vrxc')

    _classname_row = UIField('_classname_row', 'Class Name Row', desc='Use rows between 0-17', field_type = UIFieldType.BASIC_INT, value=1)
    rhapi.fields.register_option(_classname_row, 'elrs_vrxc')

    _eventname_row = UIField('_eventname_row', 'Event Name Row', desc='Use rows between 0-17', field_type = UIFieldType.BASIC_INT, value=0)
    rhapi.fields.register_option(_eventname_row, 'elrs_vrxc')

    _announcement_row = UIField('_announcement_row', 'Announcement Row', desc='Use rows between 0-17', field_type = UIFieldType.BASIC_INT, value=3)
    ...
    rhapi.ui.register_quickbutton('elrs_settings', 'rescan', "Backpack Rescan", controller.connection_search, args={"from_ui" : True})
